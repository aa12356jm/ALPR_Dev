...
Mat colorMatch(const Mat &src, Mat &match, const Color r,
  ...
  //hsv模型，hue色调范围为0-360，saturation饱和度和value亮度范围为0-1
  //h为0，红色；h为120，绿色；h为240蓝色
  // H range of blue ，经实验得到，在hue色调为200到280则认为是蓝色车牌的区域；
  ...
  // convert to HSV space，转换到hsv空间
  cvtColor(src, src_hsv, CV_BGR2HSV);

  std::vector<cv::Mat> hsvSplit;
  //通道分离
  split(src_hsv, hsvSplit);
  //对v亮度通道进行直方图均衡化，提高亮度和对比度
  ...
  float avg_h = min_h + diff_h;//中间平均色调
...
//求两个矩形的内部矩形，比如a矩形在b的内部，则返回的矩形为a
...
//求两个矩形的外部矩形，比如a矩形在b的内部，则返回的矩形为b
...
//解决方案是实现一个安全的计算最小外接矩形Rect的函数，在boundingRect()结果之上，对角点坐标进行一次判断，
//如果值为负数，就置为0，如果值超过了原始Mat的rows或cols，就置为原始Mat的这些rows或cols。
bool calcSafeRect(const RotatedRect &roi_rect, const Mat &src,Rect_<float> &safeBoundRect) {
  ...
  //boudRect左上角x和y坐标可能会小于0
  float tl_x = boudRect.x > 0 ? boudRect.x : 0;
  float tl_y = boudRect.y > 0 ? boudRect.y : 0;

  //boudRect右下角的x和y坐标可能会大于src
